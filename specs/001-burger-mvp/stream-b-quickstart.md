...
- Среди **первых пяти** элементов есть
...
Fail: маппинг museum→site; пара отсутствует в топ-5; прогон против
непустой `cluster` без живого прохода дисков.

Percent-encode: в JSON id **сырой**. Кодирование UTF-8 всего id нужно только
если D кладёт его в URL шеринга.

## 3. SC-B2 — запасной одноузловой бургер в топ-5
...
Pass: в топ-5 есть `cluster_id` из `fixtures/backup_single_hub.json`
(`c:Ярославль|Ярославская область`). Пара Ярославль+Ростов не обязана быть
первой.
...
Повторить шаги 2–3 на фикстурах. После подмены волной 1 (D1+D3) — те же два
POST без смены тел. Передача того же `cluster_id` в `/api/price` — приёмка C,
не этот quickstart.
