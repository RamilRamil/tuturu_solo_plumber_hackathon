...
| Testing | Ручной сценарий [stream-d-quickstart.md](stream-d-quickstart.md); UI не переранжирует; клик пары |
...
| 10–16 | Фаза 1: карта + карточки (до 5 видимых), **без цен**. UI не сортирует; клик пары | UI-гейт SC-D2 |
...
1. Эталон №1: UI не переранжирует; для SSE кликнуть карточку пары (`etalon_1.json`), даже если она не #1 и не в видимом топ-5 мока.
2. Запасной: карточка одноузлового Ярославля кликабельна в порядке ответа; топ-5 мока не гейт.
